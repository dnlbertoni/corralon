...
{
    function DatosCliente($nombre, $doc, $respiva, $tipdoc)
    {//Datos del cliente
    ...
    function AbrirFactura($tipdoc)
    { //abro docuemnto factura tipdoc = "A" o "B"
        $command = "@" . $this->fs . $tipdoc . $this->fs . "T" . "\n";
        $estado = $this->EscriboArchivoMandar($command);
        return $estado;
    }

    function TextoFactura()
    {    //Texto Fiscal documento factura
    ...
    {    //Subtotal documento ticket
        $command = "C" . $this->fs . "P" . $this->fs . "Subtotal" . $this->fs . $this->display . "\n";
        $estado = $this->EscriboArchivoMandar($command);
        return $estado;
    }

    function TotalFactura($total)
    {    //Total documento factura
